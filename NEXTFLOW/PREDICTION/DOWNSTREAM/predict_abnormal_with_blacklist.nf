process PRECI_ABNORMAL_WITH_BLACKLIST {
    tag "${sample_name}"
    ...
    input:
    tuple val(sample_name), path (npz_file), path (blacklist)
    path ref_npz
    val outdir
    ...
    script:
    """
    ${projectDir}/bin/preci_abnormal.py $npz_file --ref ${ref_npz} --blacklist $blacklist
    """
}
