...
def flush(readBuff):
	...
	if stairSize <= threshold or threshold < 0:	
		for read in readBuff:
			...
			# bin = location/binsize
			true_bin = location/binsize  ## float
			tentative = int(round(true_bin))  ## old bin

			# try:
			# 	assert (tentative - true_bin < 0.001)
			# 	print(tentative - true_bin,'it worked')
			# except AssertionError:
			# 	print(tentative, true_bin)
			# 	exit - 1

			if (chromosome in chromosomes):
				while len(chromosomes[chromosome]) <= bin:
					chromosomes[chromosome].append(0.)
				chromosomes[chromosome][int(bin)] += 1
		if args.keepfile:
