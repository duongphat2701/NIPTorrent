...
def correct(sample, gcCount, binSize, maxN=0.1, minRD=0.0001, fVal=0.1, iVal=3):
    ...
    print ("length of arrayX: ", len(allX))
    print ("length of arrayY:", len(allY))
    
    allX = np.array(allX, float)
    allY = np.array(allY, float)
    lowessCurve = sm.lowess(allY, allX,frac=fVal, it=iVal,return_sorted = False).tolist()
...
if __name__ == "__main__":
    ...
    # argsKeys.sort()
    # for arg in argsKeys:
    for arg in sorted(argsDict.keys()):
