...
#   python nuclDetector.py INDIR/merge.CHROM  INDIR/nucl_ex4.CHROM

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT_NUCDEC="${SCRIPT_NUCDEC:-$SCRIPT_DIR/nuclDetector.py}"
SCRIPT_PYTHON="${SCRIPT_PYTHON:-python}"

INDIR="$(readlink -f "${1:?Usage: nuclDetector.sh <train_dir>}")"

for CHROM in $(seq 22 -1 1); do
  IN="$INDIR/merge.$CHROM"
  OUT="$INDIR/nucl_ex4.$CHROM"
